"""
Streamlit App: Bajaj Finserv AMC Factsheet Chatbot (RAG PoC)

Features:
- Upload one or more Bajaj factsheets (PDF)
- Ingest text + tables (PyPDF2 + pdfplumber), OCR fallback (pytesseract)
- Build FAISS index with OpenAI embeddings (supports EURI proxy)
- Query via RAG: retrieve top-k chunks, enforce "only use provided facts"
- Computation helpers: CAGR, Sharpe
- Visuals: show source table, confidence scores, fund comparison charts
"""

# ======= Requirements =======
# pip install streamlit python-dotenv PyPDF2 pdfplumber pytesseract pillow \
#     langchain-openai langchain faiss-cpu transformers torch pandas numpy matplotlib
